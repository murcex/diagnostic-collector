namespace PlyQor.Engine.Components.Validation.Internals
{
	...
	public class InternalValidationService
	{
		...
		/// <summary>
		/// Check if system admin token is valid.
		/// </summary>
		public static bool CheckAdminToken(string token)
		{
			if (string.Equals(Configuration.SystemAdminToken, token))
			{
				return true;
			}
		...
		/// <summary>
		/// Check if system operation is valid.
		/// </summary>
		public static bool CheckSystemOperation(string operation)
		{
			if (Configuration.SystemOperations.Contains(operation))
			{
				return true;
			}
